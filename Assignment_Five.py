...
if ((len(my_password)) >= min_length) and ((len(my_password)) <= max_length):
    print("The password is", len(my_password), "characters, which is between", min_length, "to", max_length)
else:
    print("This password is invalid. Disallowed string length. ")
    print("The password is", len(my_password), "characters, which is not between", min_length, "to", max_length)
    exit()

# Check if first or last character is a hastag
if (required_symbol in my_password[1:-1]) == False:
    print("This password is invalid.  Hashtag (#) is required in password that is not the first or last character.")
    exit()
...
# print(lower_password.find(bad_string))      # Is umgc in the password?
if lower_password.find(bad_string) != -1:   #  -1 means that umgc is not found
    print("This is a valid password.  Good job!")
#    print("Password in lower case:", lower_password)
#    print("The invalid string:", bad_string)
#    valid_password_flag = valid_password_flag + 1
#    print("Flag counter:", valid_password_flag)
else:
